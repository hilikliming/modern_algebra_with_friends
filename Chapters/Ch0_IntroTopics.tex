...
If you see a mistake in this document, or want further clarification somewhere, please open an issue on the book's \href{https://github.com/hilikliming/modern_algebra_with_friends}{\textul{github repository}}, or send me an email at jack.hall618@gmail.com \\
\noindent
\begin{tcolorbox}
    \begin{center}
...
\begin{selfcheckbox}{0.1}{sec:LogicalConnectives}
  \hypertarget{q:ch0:notation}{}
  Q1. \textit{[T/F]} The statement ``$A$ only if $B$'' has the same meaning as ``$A \implies B$.''\par\smallskip
  Q2. \textit{[T/F]} The statement ``$B$ is necessary for $A$'' has the same meaning as ``$A \implies B$.''\par\smallskip
  Q3. \textit{[T/F]} ``$A$ is sufficient for $B$'' and ``$B$ is necessary for $A$'' mean the same thing.\par\smallskip
  Q4. \textit{Produce.} Using the vertical bar ``for which'' notation introduced in this section, write the set of integers $x$ that are greater than or equal to $22$ and strictly less than $27$.\par\smallskip
  Q5. \textit{Produce.} Translate each of the following into a sentence of plain English, naming each logic-and-set symbol as you go:
  \begin{enumerate}[(a),topsep=2pt,itemsep=0pt]
    \item $\forall \, x \in \Z, \; x^2 \geq 0$
    \item $\exists \, x \in \Z \ni x + 5 = 0$
  \end{enumerate}
  \par\medskip\hfill\sclink{a:ch0:notation}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.2}{sec:NegatingConnectives}
  \hypertarget{q:ch0:truthtables}{}
  Q1. \textit{[T/F]} When $P$ is false, ``if $P$ then $Q$'' is true regardless of what $Q$ is.\par\smallskip
  Q2. \textit{[T/F]} ``If $A$ then $B$'' is itself a statement whose truth value can be evaluated.\par\smallskip
  Q3. \textit{[T/F]} $P \implies Q$ has the same truth table as $\lnot P \lor Q$.\par\smallskip
  Q4. \textit{Produce.} Write a one-sentence ``for all'' statement whose domain is empty, and explain in one line why it is automatically true.
  \par\medskip\hfill\sclink{a:ch0:truthtables}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.3}{sec:axioms}
  \hypertarget{q:ch0:negations}{}
  Q1. \textit{[T/F]} The negation of ``$A$ or $B$'' is ``$\lnot A$ and $\lnot B$.''\par\smallskip
  Q2. \textit{Produce.} Write the negation of ``for every $x \in \Z$, $x \geq 0$.''\par\smallskip
  Q3. \textit{Produce.} Write the negation of ``if it is raining, then the sidewalk is wet.''\par\smallskip
  Q4. \textit{Produce.} Write the negation of ``$A$ or ($B$ and $C$),'' applying DeMorgan as many times as needed until each ``$\lnot$'' sits on a single proposition variable.
  \par\medskip\hfill\sclink{a:ch0:negations}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.4}{sec:BasicSetTheory}
  \hypertarget{q:ch0:axioms}{}
  Q1. \textit{[T/F]} An ``axiom'' in mathematics is a statement we adopt as true without justification, to serve as the starting point of an argument.\par\smallskip
  Q2. \textit{[T/F]} The sets $\{1, 2, 2, 3\}$ and $\{1, 2, 3\}$ are equal.\par\smallskip
  Q3. \textit{[T/F]} The set $\{\text{green}, \text{red}, \text{blue}, \text{red}, \text{orange}\}$ has 5 elements.\par\smallskip
  Q4. \textit{Produce.} This section lists four arithmetic facts about $\Z$: that $a + b$, $a - b$, $-a$, and $a \cdot b$ are integers whenever $a, b \in \Z$. Name one familiar arithmetic operation on two integers whose result is \emph{not} guaranteed to be an integer, with a counterexample.\par\smallskip
  Q5. \textit{[T/F]} This book treats ``set'' and ``integer'' as primitive notions: their existence and basic properties are taken as starting points and not derived from anything more fundamental.
  \par\medskip\hfill\sclink{a:ch0:axioms}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.5}{sec:ImportantSets}
  \hypertarget{q:ch0:sets}{}
  Q1. \textit{[T/F]} If $A \subset B$ and $B \subset A$, then $A = B$.\par\smallskip
  Q2. \textit{[T/F]} $\emptyset \subset X$ for every set $X$.\par\smallskip
  Q3. \textit{[T/F]} ``$A \cap B = \emptyset$'' means $A$ and $B$ share no elements in common.\par\smallskip
  Q4. \textit{[T/F]} If $A$ has 2 elements and $B$ has 3 elements, then $A \times B$ has 6 elements.\par\smallskip
  Q5. \textit{Produce.} Given $A = \{1, 2, 3\}$ and $B = \{2, 3, 4\}$, list the elements of $A \cup B$, $A \cap B$, and $A - B$.\par\smallskip
  Q6. \textit{Produce.} Using set-builder notation, write the Cartesian product $A \times B$ for arbitrary sets $A$ and $B$.\par\smallskip
  Q7. \textit{[T/F]} The difference set $A - B$ contains exactly those elements that are in $A$ but \emph{not} in $B$.
  \par\medskip\hfill\sclink{a:ch0:sets}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.6}{sec:ManipulatingInequalities}
  \hypertarget{q:ch0:impsets}{}
  Q1. \textit{[T/F]} The interval $[a, b)$ contains $a$ but does not contain $b$.\par\smallskip
  Q2. \textit{[T/F]} $\tfrac{1}{2} \in \Q$ and $\tfrac{1}{2} \in \R$, but $\tfrac{1}{2} \notin \Z$.\par\smallskip
  Q3. \textit{Produce.} Write the set of real numbers $r$ satisfying $-3 \leq r < 5$ in interval notation.\par\smallskip
  Q4. \textit{[T/F]} Every natural number is an integer, every integer is a rational number, and every rational number is a real number.
  \par\medskip\hfill\sclink{a:ch0:impsets}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.7}{sec:ContrapositiveExample}
  \hypertarget{q:ch0:ineq}{}
  Q1. \textit{[T/F]} If $a \geq b$ and $c \leq 0$, then $ac \leq bc$.\par\smallskip
  Q2. \textit{[T/F]} If $0 < a \leq b$, then $\tfrac{1}{a} \geq \tfrac{1}{b}$.\par\smallskip
  Q3. \textit{Produce.} Solve $-3x \geq 12$ for $x$, and identify the step where the inequality flips direction.\par\smallskip
  Q4. \textit{Produce.} Given $0 < 2 < 5$, write down the correct chain of inequalities between $0$, $\tfrac{1}{5}$, and $\tfrac{1}{2}$. Cite the rule(s) from this section's table that justify the ordering you chose.
  \par\medskip\hfill\sclink{a:ch0:ineq}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.8}{sec:RAAExample}
  \hypertarget{q:ch0:contrapos}{}
  Q1. \textit{[T/F]} The contrapositive of ``if $A$ then $B$'' is ``if not $B$ then not $A$,'' and the two are logically equivalent in classical logic.\par\smallskip
  Q2. \textit{[T/F]} To prove $A \implies B$ by contrapositive, we assume $\lnot B$ and derive $\lnot A$.\par\smallskip
  Q3. \textit{Produce.} State the contrapositive of ``if $n$ is an even integer, then $n + 1$ is odd.''
  \par\medskip\hfill\sclink{a:ch0:contrapos}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.9}{sec:InductionExample}
  \hypertarget{q:ch0:contra}{}
  Q1. \textit{[T/F]} To prove $A \implies B$ by contradiction, we assume $A$ is true and $B$ is false, then derive a contradiction.\par\smallskip
  Q2. \textit{Produce.} A friend says ``proof by contradiction starts by assuming the negation of the implication.'' Restate that assumption more carefully: which two things, exactly, do you assume?
  \par\medskip\hfill\sclink{a:ch0:contra}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.10}{sec:InductionPitfall}
  \hypertarget{q:ch0:induction}{}
  Q1. \textit{[T/F]} An induction proof has two parts: a base case and an induction step.\par\smallskip
  Q2. \textit{[T/F]} The base case of an induction proof must always be $n = 0$.\par\smallskip
  Q3. \textit{[T/F]} In the induction step, we assume the statement holds for some $n \geq N$ and show it holds for $n + 1$.\par\smallskip
  Q4. \textit{Produce.} In the induction proof of $\sum_{k=1}^n k = \tfrac{n(n+1)}{2}$ given in this section, what is the base case used, and what does the statement say there?
  \par\medskip\hfill\sclink{a:ch0:induction}{answers}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.12}{sec:ChongInduction}
  \hypertarget{q:ch0:whyinduction}{}
  Q1. \textit{[T/F]} The informal ``$\ldots$'' in the chain $P(N), P(N+1), P(N+2), \ldots$ implicitly appeals to induction itself, so it cannot serve as the justification for induction.\par\smallskip
  Q2. \textit{[T/F]} The contrapositive-style argument in this section shows that if the base case and induction step both hold, then there cannot exist a smallest $k \geq N$ for which $P(k)$ fails.\par\smallskip
  Q3. \textit{Produce.} The section's argument denies the existence of a ``first $k$'' for which $P$ fails. Why is denying the existence of a \emph{first} failure enough to conclude $P$ holds for every $n \geq N$?
  \par\medskip\hfill\sclink{a:ch0:whyinduction}{answers}
\end{selfcheckbox}
...
\hypertarget{a:ch0:notation}{}
\noindent Q1. T. ``$A$ only if $B$'' is one of the five standard readings of $A \implies B$ given in this section; the antecedent is $A$, not $B$.\par\smallskip
\noindent Q2. T. ``Necessary'' tracks the consequent of an implication; ``sufficient'' tracks the antecedent.\par\smallskip
\noindent Q3. T. Both are readings of $A \implies B$: ``sufficient'' tags the antecedent ($A$ on its own is enough to force $B$), and ``necessary'' tags the consequent ($B$ has to hold for $A$ to possibly hold). Reading them paired makes the equivalence obvious.\par\smallskip
\noindent Q4. $\{x \in \Z \mid 22 \leq x < 27\} = \{22, 23, 24, 25, 26\}$. The vertical bar reads as ``for which.''\par\smallskip
\noindent Q5. (a) ``For every integer $x$, $x^2$ is at least $0$'' (true). Symbols: $\forall$ is ``for every,'' $\in$ is ``is an element of,'' $\Z$ names the integers. (b) ``There exists an integer $x$ for which $x + 5 = 0$'' (true, namely $x = -5$). Symbols: $\exists$ is ``there exists,'' $\ni$ is the element-wise ``for which.'' A reading convention: the comma after the quantification clause (as in $\forall x \in \Z, \, \ldots$) just separates the quantifier-and-domain from the body, so $\forall x \in \Z, P(x)$ reads ``for every $x \in \Z$, $P(x)$ holds.''\par\smallskip
\hfill\sclink{q:ch0:notation}{back}\par\medskip

\subsection*{\S\ref{sec:LogicalConnectives} Logical Connectives \& Truth Tables}
\hypertarget{a:ch0:truthtables}{}
\noindent Q1. T. This is \textit{vacuous truth}: an implication with a false antecedent is satisfied with nothing to check. The Purple Motorcycle Club is this section's vehicle for the idea.\par\smallskip
\noindent Q2. T. The whole ``if-then'' expression has its own truth value; it is not meta-machinery outside the logical world. ``Charlie is a human implies Charlie was born on Earth'' is itself either true or false.\par\smallskip
\noindent Q3. T. The two forms agree on every row of the truth table; this section calls the equivalence out explicitly.\par\smallskip
\noindent Q4. Several examples work, each with an empty domain: (a) ``For every $n \in \N$ with $n < 0$, $n$ is one of my uncles'' (no natural numbers are negative). (b) ``All of my motorcycles are purple,'' when you own zero motorcycles. (c) ``Every element of $\emptyset$ is a number.'' In each case the ``for all'' quantifier has nothing to check, so the statement holds by default.\par\smallskip
\hfill\sclink{q:ch0:truthtables}{back}\par\medskip

\subsection*{\S\ref{sec:NegatingConnectives} Negating Connectives \& Quantifiers}
\hypertarget{a:ch0:negations}{}
\noindent Q1. T. This is one of DeMorgan's laws (the ``or''-negation form); the negation of an ``or'' is the ``and'' of the negations.\par\smallskip
\noindent Q2. $\exists \, x \in \Z \ni x < 0$. (Equivalently in words: ``there exists an integer $x$ for which $x < 0$.'' Negating a universal gives an existential of the negated predicate.)\par\smallskip
\noindent Q3. ``It is raining and the sidewalk is not wet.'' The negation of ``if $A$ then $B$'' is ``$A$ and $\lnot B$,'' per the third row of this section's negation table.\par\smallskip
\noindent Q4. $\lnot(A \lor (B \land C)) = \lnot A \land \lnot(B \land C)$ (DeMorgan applied to the outer ``or''), and then $\lnot(B \land C) = \lnot B \lor \lnot C$ (DeMorgan applied to the inner ``and''). So the fully simplified negation is $\lnot A \land (\lnot B \lor \lnot C)$.\par\smallskip
\hfill\sclink{q:ch0:negations}{back}\par\medskip

\subsection*{\S\ref{sec:axioms} A Word About Axioms}
\hypertarget{a:ch0:axioms}{}
\noindent Q1. T. This is the working definition of ``axiom'' adopted by this book: statements taken as true without justification, serving as the starting point of argument.\par\smallskip
\noindent Q2. T. Sets have no duplicity: an element is either in a set or it is not, with no notion of multiple copies. So $\{1,2,2,3\} = \{1,2,3\}$.\par\smallskip
\noindent Q3. F. The repeated $\text{red}$ does not add a new element. The set has 4 elements: $\text{green}, \text{red}, \text{blue}, \text{orange}$.\par\smallskip
\noindent Q4. Division is the standard answer: $1 / 2 \notin \Z$. Exponentiation works too: $2^{-1} = 1/2 \notin \Z$. (The four listed facts cover $+$, $-$, additive inverses, and $\cdot$; everything else is fair game for leaving $\Z$ behind.)\par\smallskip
\noindent Q5. T. The section explicitly says this book is not a foundations course; sets and the integers $\Z$ (with the listed arithmetic facts) are taken as primitive, with no further justification attempted.\par\smallskip
\hfill\sclink{q:ch0:axioms}{back}\par\medskip

\subsection*{\S\ref{sec:BasicSetTheory} Basic Set Theory Definitions}
\hypertarget{a:ch0:sets}{}
\noindent Q1. T. This is the definition of set equality used in this book (and the standard one): two sets are equal exactly when each is a subset of the other.\par\smallskip
\noindent Q2. T. The statement ``every element of $\emptyset$ is also in $X$'' is vacuously true: the empty set has no elements, so the ``every'' has nothing to check. This is the same vacuous-truth pattern as the Purple Motorcycle Club in \S\ref{sec:LogicalConnectives}.\par\smallskip
\noindent Q3. T. By the section's definition: $A$ and $B$ are disjoint exactly when their intersection is the empty set, i.e., they share no elements.\par\smallskip
\noindent Q4. T. The section states $|A \times B| = |A| \cdot |B|$ when both sets are finite. Concretely: $2 \cdot 3 = 6$ ordered pairs.\par\smallskip
\noindent Q5. $A \cup B = \{1, 2, 3, 4\}$ (elements in either); $A \cap B = \{2, 3\}$ (elements in both); $A - B = \{1\}$ (elements in $A$ but not in $B$).\par\smallskip
\noindent Q6. $A \times B = \{(a, b) \mid a \in A \text{ and } b \in B\}$. This is the section's definition of the Cartesian product: the set of all ordered pairs with first coordinate in $A$ and second coordinate in $B$.\par\smallskip
\noindent Q7. T. By the section's definition: $A - B = \{x \in A \mid x \notin B\}$. Equivalent to ``the part of $A$ left over after you remove everything also in $B$.''\par\smallskip
\hfill\sclink{q:ch0:sets}{back}\par\medskip

\subsection*{\S\ref{sec:ImportantSets} Important Sets \& Interval Notation}
\hypertarget{a:ch0:impsets}{}
\noindent Q1. T. A square bracket includes its endpoint; a round bracket excludes it. So $a \in [a,b)$ but $b \notin [a,b)$.\par\smallskip
\noindent Q2. T. $\tfrac{1}{2}$ is rational ($a/b$ with $a=1, b=2$ and $b \neq 0$) and so also real, but it is not an integer.\par\smallskip
\noindent Q3. $[-3, 5)$. The left endpoint is included ($-3 \leq r$, square bracket); the right endpoint is excluded ($r < 5$, round bracket).\par\smallskip
\noindent Q4. T. The familiar nesting chain is $\N \subset \Z \subset \Q \subset \R$. Each containment is strict: $-1 \in \Z$ but $-1 \notin \N$; $\tfrac{1}{2} \in \Q$ but $\tfrac{1}{2} \notin \Z$; $\sqrt{2} \in \R$ but $\sqrt{2} \notin \Q$.\par\smallskip
\hfill\sclink{q:ch0:impsets}{back}\par\medskip

\subsection*{\S\ref{sec:ManipulatingInequalities} Manipulating Inequalities}
\hypertarget{a:ch0:ineq}{}
\noindent Q1. T. The Multiplication rule in the section's table: multiplying both sides of $a \geq b$ by a non-positive $c$ flips the inequality, giving $ac \leq bc$.\par\smallskip
\noindent Q2. T. The Reciprocals rule: for $0 < a \leq b$, the reciprocals reverse the inequality, so $\tfrac{1}{a} \geq \tfrac{1}{b}$ (smaller positive number gives larger reciprocal).\par\smallskip
\noindent Q3. Divide both sides by $-3$ (equivalently, multiply by $-\tfrac{1}{3}$): $x \leq -4$. The flip happens precisely because we multiplied/divided by a negative quantity (the Multiplication or Division rule with $c < 0$).\par\smallskip
\noindent Q4. $0 < \tfrac{1}{5} < \tfrac{1}{2}$. Three pieces from the table combine: the third Reciprocals row ($0 < a \leq b \implies \tfrac{1}{a} \geq \tfrac{1}{b}$, applied with $a=2, b=5$) gives $\tfrac{1}{5} < \tfrac{1}{2}$; the first Reciprocals row ($a > 0 \implies \tfrac{1}{a} > 0$) gives $0 < \tfrac{1}{5}$; and Transitivity chains these into the single inequality $0 < \tfrac{1}{5} < \tfrac{1}{2}$.\par\smallskip
\hfill\sclink{q:ch0:ineq}{back}\par\medskip

\subsection*{\S\ref{sec:ContrapositiveExample} Proof by Contrapositive}
\hypertarget{a:ch0:contrapos}{}
\noindent Q1. T. Both parts are the substance of this section: the contrapositive of $A \implies B$ is $\lnot B \implies \lnot A$, and they share a truth table.\par\smallskip
\noindent Q2. T. That is exactly the structure of a contrapositive proof: assume the negation of the consequent, then derive the negation of the antecedent.\par\smallskip
\noindent Q3. ``If $n+1$ is not odd, then $n$ is not even.'' Equivalently: ``If $n+1$ is even, then $n$ is odd.'' (Negate the consequent, then the antecedent, and swap their order.)\par\smallskip
\hfill\sclink{q:ch0:contrapos}{back}\par\medskip

\subsection*{\S\ref{sec:RAAExample} Proof by Contradiction}
\hypertarget{a:ch0:contra}{}
\noindent Q1. T. That is the structure of a proof by contradiction in classical logic: assume $A \land \lnot B$, derive $\Rightarrow \Leftarrow$, conclude $A \implies B$. (As this section's footnote notes, the move relies on classical-logic principles like indirect proof; intuitionistic logic treats such proofs differently.)\par\smallskip
\noindent Q2. You assume $A$ (the antecedent of the original implication) AND $\lnot B$ (the negation of the consequent). Saying ``assume the negation of $A \implies B$'' is loose shorthand: per the negation table in \S\ref{sec:NegatingConnectives}, the negation of ``if $A$ then $B$'' is ``$A$ and $\lnot B$,'' so the two assumptions are precisely that pair.\par\smallskip
\hfill\sclink{q:ch0:contra}{back}\par\medskip

\subsection*{\S\ref{sec:InductionExample} Proof by Induction Example}
\hypertarget{a:ch0:induction}{}
\noindent Q1. T. The two parts together establish $P(n)$ for all $n \geq N$.\par\smallskip
\noindent Q2. F. The base case can be any natural number $N$; the conclusion is then that $P(n)$ holds for all $n \geq N$. The section's sum-of-integers example uses $n = 1$ as its base.\par\smallskip
\noindent Q3. T. That is the structure of the induction step.\par\smallskip
\noindent Q4. The section uses $n = 1$ as the base case. There the statement says $\sum_{k=1}^1 k = 1 = \tfrac{1 \cdot (1+1)}{2} = 1$, which holds by inspection.\par\smallskip
\hfill\sclink{q:ch0:induction}{back}\par\medskip

\subsection*{\S\ref{sec:WhyInductionWorks} Why Induction Works}
\hypertarget{a:ch0:whyinduction}{}
\noindent Q1. T. The ``$\ldots$'' silently invokes the very induction we are trying to validate; that is the circular reasoning this section warns about.\par\smallskip
\noindent Q2. T. That is the structure of the section's proof: from the base case and induction step, no first failure can exist.\par\smallskip
\noindent Q3. If $P$ failed at any $n \geq N$, the set of failures would be a non-empty subset of the natural numbers, and every non-empty subset of $\N$ has a smallest member (this is a foundational property of $\N$ sometimes called the \textit{well-ordering principle}). So if no \emph{smallest} failure can exist, no failure can exist at all: the failure set is empty, and $P$ holds for every $n \geq N$.\par\smallskip
\hfill\sclink{q:ch0:whyinduction}{back}\par\medskip
