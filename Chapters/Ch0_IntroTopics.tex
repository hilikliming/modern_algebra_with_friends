...
\hypertarget{a:ch0:notation:1}{Q1.} T. ``$A$ only if $B$'' is one of the five standard readings of $A \implies B$ given in this section; the antecedent is $A$, not $B$. \sclink{q:ch0:notation:1}{back}\par\smallskip
\hypertarget{a:ch0:notation:2}{Q2.} T. ``Necessary'' tracks the consequent of an implication; ``sufficient'' tracks the antecedent. \sclink{q:ch0:notation:2}{back}\par\smallskip
\hypertarget{a:ch0:notation:3}{Q3.} T. This is the definition of the contrapositive; in classical logic, $A \implies B$ and $\lnot B \implies \lnot A$ are logically equivalent. \sclink{q:ch0:notation:3}{back}\par\smallskip
\hypertarget{a:ch0:notation:4}{Q4.} $\{x \in \Z \mid 22 \leq x < 27\} = \{22, 23, 24, 25, 26\}$. The vertical bar reads as ``for which.'' \sclink{q:ch0:notation:4}{back}\par\smallskip

\subsection*{\S\ref{sec:LogicalConnectives} Logical Connectives \& Truth Tables}
\hypertarget{a:ch0:truthtables:1}{Q1.} T. This is \textit{vacuous truth}: an implication with a false antecedent is satisfied with nothing to check. The Purple Motorcycle Club is this section's vehicle for the idea. \sclink{q:ch0:truthtables:1}{back}\par\smallskip
\hypertarget{a:ch0:truthtables:2}{Q2.} T. The whole ``if-then'' expression has its own truth value; it is not meta-machinery outside the logical world. ``Charlie is a human implies Charlie was born on Earth'' is itself either true or false. \sclink{q:ch0:truthtables:2}{back}\par\smallskip
\hypertarget{a:ch0:truthtables:3}{Q3.} T. The two forms agree on every row of the truth table; this section calls the equivalence out explicitly. \sclink{q:ch0:truthtables:3}{back}\par\smallskip
\hypertarget{a:ch0:truthtables:4}{Q4.} Example: ``All of my motorcycles are purple,'' when you own no motorcycles; or ``Every element of $\emptyset$ is a number.'' Both are universal statements whose domains are empty, so each holds by default. \sclink{q:ch0:truthtables:4}{back}\par\smallskip

\subsection*{\S\ref{sec:NegatingConnectives} Negating Connectives \& Quantifiers}
\hypertarget{a:ch0:negations:1}{Q1.} T. This is one of DeMorgan's laws (the ``or''-negation form); the negation of an ``or'' is the ``and'' of the negations. \sclink{q:ch0:negations:1}{back}\par\smallskip
\hypertarget{a:ch0:negations:2}{Q2.} $\exists \, x \in \Z \ni x < 0$. (Equivalently in words: ``there exists an integer $x$ for which $x < 0$.'' Negating a universal gives an existential of the negated predicate.) \sclink{q:ch0:negations:2}{back}\par\smallskip
\hypertarget{a:ch0:negations:3}{Q3.} ``It is raining and the sidewalk is not wet.'' The negation of ``if $A$ then $B$'' is ``$A$ and $\lnot B$,'' per the third row of this section's negation table. \sclink{q:ch0:negations:3}{back}\par\smallskip

\subsection*{\S\ref{sec:axioms} A Word About Axioms}
\hypertarget{a:ch0:axioms:1}{Q1.} T. This is the working definition of ``axiom'' adopted by this book: statements taken as true without justification, serving as the starting point of argument. \sclink{q:ch0:axioms:1}{back}\par\smallskip
\hypertarget{a:ch0:axioms:2}{Q2.} T. Sets have no duplicity: an element is either in a set or it is not, with no notion of multiple copies. So $\{1,2,2,3\} = \{1,2,3\}$. \sclink{q:ch0:axioms:2}{back}\par\smallskip
\hypertarget{a:ch0:axioms:3}{Q3.} F. The repeated $\text{red}$ does not add a new element. The set has 4 elements: $\text{green}, \text{red}, \text{blue}, \text{orange}$. \sclink{q:ch0:axioms:3}{back}\par\smallskip
\hypertarget{a:ch0:axioms:4}{Q4.} Division: $1 / 2 \notin \Z$. (The four facts list $+$, $-$, additive-inverse existence, and $\cdot$; division is conspicuously absent. Division of two integers can land outside $\Z$, as the $1/2$ example shows.) \sclink{q:ch0:axioms:4}{back}\par\smallskip

\subsection*{\S\ref{sec:BasicSetTheory} Basic Set Theory Definitions}
\hypertarget{a:ch0:sets:1}{Q1.} T. This is the definition of set equality used in this book (and the standard one): two sets are equal exactly when each is a subset of the other. \sclink{q:ch0:sets:1}{back}\par\smallskip
...
\hypertarget{a:ch0:sets:3}{Q3.} T. By the section's definition: $A$ and $B$ are disjoint exactly when their intersection is the empty set, i.e., they share no elements. \sclink{q:ch0:sets:3}{back}\par\smallskip
\hypertarget{a:ch0:sets:4}{Q4.} T. The section states $|A \times B| = |A| \cdot |B|$ when both sets are finite. Concretely: $2 \cdot 3 = 6$ ordered pairs. \sclink{q:ch0:sets:4}{back}\par\smallskip
\hypertarget{a:ch0:sets:5}{Q5.} $A \cup B = \{1, 2, 3, 4\}$ (elements in either); $A \cap B = \{2, 3\}$ (elements in both); $A - B = \{1\}$ (elements in $A$ but not in $B$). \sclink{q:ch0:sets:5}{back}\par\smallskip
\hypertarget{a:ch0:sets:6}{Q6.} $A \times B = \{(a, b) \mid a \in A \text{ and } b \in B\}$. This is the section's definition of the Cartesian product: the set of all ordered pairs with first coordinate in $A$ and second coordinate in $B$. \sclink{q:ch0:sets:6}{back}\par\smallskip

\subsection*{\S\ref{sec:ImportantSets} Important Sets \& Interval Notation}
\hypertarget{a:ch0:impsets:1}{Q1.} T. A square bracket includes its endpoint; a round bracket excludes it. So $a \in [a,b)$ but $b \notin [a,b)$. \sclink{q:ch0:impsets:1}{back}\par\smallskip
\hypertarget{a:ch0:impsets:2}{Q2.} T. $\tfrac{1}{2}$ is rational ($a/b$ with $a=1, b=2$ and $b \neq 0$) and so also real, but it is not an integer. \sclink{q:ch0:impsets:2}{back}\par\smallskip
\hypertarget{a:ch0:impsets:3}{Q3.} $[-3, 5)$. The left endpoint is included ($-3 \leq r$, square bracket); the right endpoint is excluded ($r < 5$, round bracket). \sclink{q:ch0:impsets:3}{back}\par\smallskip

\subsection*{\S\ref{sec:ManipulatingInequalities} Manipulating Inequalities}
\hypertarget{a:ch0:ineq:1}{Q1.} T. The Multiplication rule in the section's table: multiplying both sides of $a \geq b$ by a non-positive $c$ flips the inequality, giving $ac \leq bc$. \sclink{q:ch0:ineq:1}{back}\par\smallskip
\hypertarget{a:ch0:ineq:2}{Q2.} T. The Reciprocals rule: for $0 < a \leq b$, the reciprocals reverse the inequality, so $\tfrac{1}{a} \geq \tfrac{1}{b}$ (smaller positive number gives larger reciprocal). \sclink{q:ch0:ineq:2}{back}\par\smallskip
\hypertarget{a:ch0:ineq:3}{Q3.} Divide both sides by $-3$ (equivalently, multiply by $-\tfrac{1}{3}$): $x \leq -4$. The flip happens precisely because we multiplied/divided by a negative quantity (the Multiplication or Division rule with $c < 0$). \sclink{q:ch0:ineq:3}{back}\par\smallskip
\hypertarget{a:ch0:ineq:4}{Q4.} $0 < \tfrac{1}{5} < \tfrac{1}{2}$. Reciprocals of positive numbers reverse the ordering: the smaller of $2$ and $5$ (which is $2$) gives the larger reciprocal ($\tfrac{1}{2}$), and the larger of the two ($5$) gives the smaller positive reciprocal ($\tfrac{1}{5}$). \sclink{q:ch0:ineq:4}{back}\par\smallskip

\subsection*{\S\ref{sec:ContrapositiveExample} Proof by Contrapositive}
\hypertarget{a:ch0:contrapos:1}{Q1.} T. Both parts are the substance of this section: the contrapositive of $A \implies B$ is $\lnot B \implies \lnot A$, and they share a truth table. \sclink{q:ch0:contrapos:1}{back}\par\smallskip
\hypertarget{a:ch0:contrapos:2}{Q2.} T. That is exactly the structure of a contrapositive proof: assume the negation of the consequent, then derive the negation of the antecedent. \sclink{q:ch0:contrapos:2}{back}\par\smallskip
\hypertarget{a:ch0:contrapos:3}{Q3.} ``If $n+1$ is not odd, then $n$ is not even.'' Equivalently: ``If $n+1$ is even, then $n$ is odd.'' (Negate the consequent, then the antecedent, and swap their order.) \sclink{q:ch0:contrapos:3}{back}\par\smallskip

\subsection*{\S\ref{sec:RAAExample} Proof by Contradiction}
\hypertarget{a:ch0:contra:1}{Q1.} T. That is the definition: assume $A \land \lnot B$, derive $\Rightarrow \Leftarrow$, conclude $A \implies B$. \sclink{q:ch0:contra:1}{back}\par\smallskip
...
\hypertarget{a:ch0:induction:1}{Q1.} T. The two parts together establish $P(n)$ for all $n \geq N$. \sclink{q:ch0:induction:1}{back}\par\smallskip
\hypertarget{a:ch0:induction:2}{Q2.} F. The base case can be any natural number $N$; the conclusion is then that $P(n)$ holds for all $n \geq N$. The section's sum-of-integers example uses $n = 1$ as its base. \sclink{q:ch0:induction:2}{back}\par\smallskip
\hypertarget{a:ch0:induction:3}{Q3.} T. That is the structure of the induction step. \sclink{q:ch0:induction:3}{back}\par\smallskip
\hypertarget{a:ch0:induction:4}{Q4.} The section uses $n = 1$ as the base case. There the statement says $\sum_{k=1}^1 k = 1 = \tfrac{1 \cdot (1+1)}{2} = 1$, which holds by inspection. \sclink{q:ch0:induction:4}{back}\par\smallskip

\subsection*{\S\ref{sec:WhyInductionWorks} Why Induction Works}
\hypertarget{a:ch0:whyinduction:1}{Q1.} T. The ``$\ldots$'' silently invokes the very induction we are trying to validate; that is the circular reasoning this section warns about. \sclink{q:ch0:whyinduction:1}{back}\par\smallskip
\hypertarget{a:ch0:whyinduction:2}{Q2.} T. That is the structure of the section's proof: from the base case and induction step, no first failure can exist. \sclink{q:ch0:whyinduction:2}{back}\par\smallskip
