...
\begin{tcolorbox}
    \begin{center}
\jjh{\href{https://www.dropbox.com/sh/759pwsjyc3ix0jy/AACv98R1Zpjvbaz2VhH4aH4ca?dl=0}{The Dropbox folder for this course can be found here. \\
...
\medskip
\noindent \textbf{Self-Checks.} Most sections of this chapter open with a small \textit{Self-Check}: a handful of true/false and short-answer questions that probe the section's main ideas. If you can answer them all confidently, you can safely skip to the next section; if not, read on. Answers are gathered at the end of the chapter in \S\ref{sec:ch0SelfCheckAnswers}; each question links forward to its answer, and each answer links back. Two sections, \S\ref{sec:InductionPitfall} and \S\ref{sec:ChongInduction}, have no Self-Check by design: the first is itself a puzzle you are meant to solve, and the second is a softer alternative route for readers new to induction. The authors recommend that readers who are familiar with mathematical induction already read sections \ref{sec:InductionExample} through \ref{sec:WhyInductionWorks}. For readers who have never seen induction before, as to not distract from the more important intuition, we would prefer you skip these and read \ref{sec:ChongInduction} first, then return to those sections after the underlying argument for the principle of induction is understood. Section \ref{sec:WhyInductionWorks} is a deeper-dive into \textit{why} induction works and is optional reading for the curious.

\medskip
\noindent Lastly, when reading this material, read slowly and carefully and try to explain the argument out loud as if another were listening. Don't just read it aloud, really think about what the claim or statement is asserting. There is very little redundancy in mathematical writing so it pays dividends to read slowly, carefully, and to re-read many times until the argument is so clear that you could write it down again in your own words. When I read a new book or article in my field, the first 2-3 reads through I am just letting the new notations, jargon, definitions, and claims wash over me, I want to see the way the author uses these terms first and then I try to understand them more carefully. You can do it, you can piece it together! Just take baby steps to make your foundation solid. \newpage

\section{Common Notation seen in Herstein's book and this book}
\label{sec:CommonNotation}

\begin{selfcheckbox}{0.1}{sec:LogicalConnectives}
  \hypertarget{q:ch0:notation:1}{Q1.} \textit{[T/F]} The statement ``$A$ only if $B$'' has the same meaning as ``$A \implies B$.'' \sclink{a:ch0:notation:1}{answer}\par\smallskip
  \hypertarget{q:ch0:notation:2}{Q2.} \textit{[T/F]} The statement ``$B$ is necessary for $A$'' has the same meaning as ``$A \implies B$.'' \sclink{a:ch0:notation:2}{answer}\par\smallskip
  \hypertarget{q:ch0:notation:3}{Q3.} \textit{[T/F]} ``$A$ is sufficient for $B$'' and ``$B$ is necessary for $A$'' mean the same thing. \sclink{a:ch0:notation:3}{answer}\par\smallskip
  \hypertarget{q:ch0:notation:4}{Q4.} \textit{Produce.} Using the vertical bar ``for which'' notation introduced in this section, write the set of integers $x$ that are greater than or equal to $22$ and strictly less than $27$. \sclink{a:ch0:notation:4}{answer}\par\smallskip
  \hypertarget{q:ch0:notation:5}{Q5.} \textit{Produce.} Translate each of the following into a sentence of plain English, naming each logic-and-set symbol as you go:
  \begin{enumerate}[(a),topsep=2pt,itemsep=0pt]
    \item $\forall \, x \in \Z, \; x^2 \geq 0$
    \item $\exists \, x \in \Z \ni x + 5 = 0$ \hspace{1in} \sclink{a:ch0:notation:5}{answer}
  \end{enumerate}
...
\paragraph{Reading universally quantified statements.}
A statement of the form ``$\forall x \in S, P(x)$'' is read ``for every $x$ in $S$, $P(x)$ holds.'' The comma after the quantification clause is just a separator before the body. You will sometimes see ``$\ni$'' play this role when an extra qualifier attaches directly to the variable's domain; for example, ``$\forall x \in \Z \ni x > 0, P(x)$'' reads ``for every $x \in \Z$ for which $x > 0$, $P(x)$ holds.''

\paragraph{Writing proofs of universally quantified statements.}
When you are writing a proof of a ``$\forall x \in S, P(x)$'' statement, the standard opening is ``\textit{Let $x \in S$ be arbitrary}'' (or equivalently ``\textit{Fix $x \in S$}''). The argument that follows treats $x$ as a generic representative of $S$, so any conclusion you reach about $x$ applies to every element of $S$ at once. This convention is common enough that the explicit ``$\forall x$'' is often dropped from the prose entirely, with ``Let $x \in S$'' opening the argument by itself.
...
\label{sec:LogicalConnectives}

\begin{selfcheckbox}{0.2}{sec:NegatingConnectives}
  \hypertarget{q:ch0:truthtables:1}{Q1.} \textit{[T/F]} When $P$ is false, ``if $P$ then $Q$'' is true regardless of what $Q$ is. \sclink{a:ch0:truthtables:1}{answer}\par\smallskip
  \hypertarget{q:ch0:truthtables:2}{Q2.} \textit{[T/F]} ``If $A$ then $B$'' is itself a statement whose truth value can be evaluated. \sclink{a:ch0:truthtables:2}{answer}\par\smallskip
  \hypertarget{q:ch0:truthtables:3}{Q3.} \textit{[T/F]} $P \implies Q$ has the same truth table as $\lnot P \lor Q$. \sclink{a:ch0:truthtables:3}{answer}\par\smallskip
  \hypertarget{q:ch0:truthtables:4}{Q4.} \textit{Produce.} Write a one-sentence ``for all'' statement whose domain is empty, and explain in one line why it is automatically true. \sclink{a:ch0:truthtables:4}{answer}
\end{selfcheckbox}

In propositional logic, statements (or \textit{propositions}) can be combined using
\textit{logical connectives}. We denote \textit{conjunction} (logical ``and'') by the symbol $\land$,
and \textit{disjunction} (logical ``or'') by the symbol $\lor$.
Given two propositions $P$ and $Q$:

\begin{itemize}
    \item $P \land Q$ is \textit{true} if and only if \textit{both} $P$ and $Q$ are true.
    \item $P \lor Q$ is \textit{true} if at least one of $P$ or $Q$ is true.
\end{itemize}
...
\noindent Here:
\begin{itemize}
    \item $\lnot P$ (\textit{negation}) is true when $P$ is false.
    \item $P \Rightarrow Q$ (\textit{implication/if-then}) is false only when $P$ is true and $Q$ is false.
    \item $P \Leftrightarrow Q$ (\textit{biconditional/if-and-only-if}) is true when $P$ and $Q$ have the same truth value.
\end{itemize}
...
\paragraph{Vacuous Truth (The Purple Motorcycle Club).}
\phantomsection\label{para:purple-motorcycle-club}
When $P$ is false, $P \Rightarrow Q$ is true regardless of what $Q$ is. This
surprises most readers the first time. The intuition: ``if $P$ then $Q$"
promises that whenever $P$ is true, $Q$ is also true; if $P$ is never true,
the promise has no chance to be broken, and we say it is satisfied
\textit{vacuously}\index{Vacuous Truth}. The same idea shows up under
universal quantifiers: a statement ``$\forall x \in S,\ P(x)$" amounts to a
conjunction of conditionals, one per $x$ (``if $x \in S$ then $P(x)$"). When
$S$ is empty, every antecedent is false and every conditional is true by the
rule we just discussed, so the universal holds by default with nothing to
check.

A favorite illustration of both effects (which we learned from Dr.\ Edwin
K.\,P.\ Chong) is the \textit{Purple Motorcycle Club}. Membership has exactly
one rule, ``all your motorcycles must be purple," and you happen to own zero
motorcycles. Can you join?

You can. Read the rule as a universal: ``$\forall m \in \text{your
motorcycles},\ m\text{ is purple}$." Your collection is empty, so the
universal holds vacuously. Or read it as a conditional: ``if $m$ is one of
your motorcycles, then $m$ is purple." The antecedent is never true, so the
conditional is never violated; it counts as true across the board. Either
reading lands at the same conclusion: you qualify. Had the club meant to
exclude you, they would have needed a stronger rule, something like ``you
must own at least one motorcycle, and every motorcycle you own must be
purple." That statement adds an existence requirement the original lacks.

This pattern shows up throughout the book. The fact that
$\emptyset \subseteq X$ for every set $X$ is a vacuous-truth statement
(``every element of $\emptyset$ also belongs to $X$"; there are no elements
of $\emptyset$, so the ``every" is fulfilled trivially), and the
$\forall$- and $\rightarrow$-introduction rules in the Logic Appendix rely on
the same structure. Whenever ``every," ``for all," or ``if\ldots then\ldots"
meets a domain that turns out to be empty, expect the statement to come out
true by default.
\newpage 
\section{Negating Logical Connectives and Quantifiers}
\label{sec:NegatingConnectives}

\begin{selfcheckbox}{0.3}{sec:axioms}
  \hypertarget{q:ch0:negations:1}{Q1.} \textit{[T/F]} The negation of ``$A$ or $B$'' is ``$\lnot A$ and $\lnot B$.'' \sclink{a:ch0:negations:1}{answer}\par\smallskip
  \hypertarget{q:ch0:negations:2}{Q2.} \textit{Produce.} Write the negation of ``for every $x \in \Z$, $x \geq 0$.'' \sclink{a:ch0:negations:2}{answer}\par\smallskip
  \hypertarget{q:ch0:negations:3}{Q3.} \textit{Produce.} Write the negation of ``if it is raining, then the sidewalk is wet.'' \sclink{a:ch0:negations:3}{answer}\par\smallskip
  \hypertarget{q:ch0:negations:4}{Q4.} \textit{Produce.} Write the negation of ``$A$ or ($B$ and $C$),'' applying DeMorgan as many times as needed until each ``$\lnot$'' sits on a single proposition variable. \sclink{a:ch0:negations:4}{answer}
\end{selfcheckbox}
...
\noindent The general principle behind these rules is that negation pushes inward one level at a time. When you apply a row of the table to a compound expression, the $\lnot$ moves one step deeper into the structure; if what remains under a $\lnot$ is itself compound, you apply the appropriate row again, and you repeat until each remaining $\lnot$ sits on an atomic proposition (a single proposition variable). Two worked examples make this concrete.

\noindent \textit{Example 1 (single application).} Negating $P \land Q$ once is enough, because the inner pieces $P$ and $Q$ are already atomic:
\begin{align}
    \lnot(P \land Q) &= \lnot P \lor \lnot Q \quad \text{(``and'' negation row)} \nonumber
\end{align}

\noindent \textit{Example 2 (nested application).} Negating $A \lor (B \land C)$ requires two passes; the first pulls the $\lnot$ past the outer ``or,'' the second pulls it past the inner ``and'':
\begin{align}
    \lnot(A \lor (B \land C)) &= \lnot A \land \lnot(B \land C) \quad &\text{(``or'' negation row)} \nonumber \\
    &= \lnot A \land (\lnot B \lor \lnot C) \quad &\text{(``and'' negation row applied to } \lnot(B \land C) \text{)} \nonumber
\end{align}
The result is fully simplified: each remaining $\lnot$ sits on a single proposition variable, joined by the basic connectives ``$\land$'' and ``$\lor$.''
...
\begin{selfcheckbox}{0.4}{sec:BasicSetTheory}
  \hypertarget{q:ch0:axioms:1}{Q1.} \textit{[T/F]} An ``axiom'' in mathematics is a statement we adopt as true without justification, to serve as the starting point of an argument. \sclink{a:ch0:axioms:1}{answer}\par\smallskip
  \hypertarget{q:ch0:axioms:2}{Q2.} \textit{[T/F]} The sets $\{1, 2, 2, 3\}$ and $\{1, 2, 3\}$ are equal. \sclink{a:ch0:axioms:2}{answer}\par\smallskip
  \hypertarget{q:ch0:axioms:3}{Q3.} \textit{[T/F]} The set $\{\text{green}, \text{red}, \text{blue}, \text{red}, \text{orange}\}$ has 5 elements. \sclink{a:ch0:axioms:3}{answer}\par\smallskip
  \hypertarget{q:ch0:axioms:4}{Q4.} \textit{Produce.} This section lists four arithmetic facts about $\Z$: that $a + b$, $a - b$, $-a$, and $a \cdot b$ are integers whenever $a, b \in \Z$. Name one familiar arithmetic operation on two integers whose result is \emph{not} guaranteed to be an integer, with a counterexample. \sclink{a:ch0:axioms:4}{answer}\par\smallskip
  \hypertarget{q:ch0:axioms:5}{Q5.} \textit{[T/F]} This book treats ``set'' and ``integer'' as primitive notions: their existence and basic properties are taken as starting points and not derived from anything more fundamental. \sclink{a:ch0:axioms:5}{answer}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.5}{sec:ImportantSets}
  \hypertarget{q:ch0:sets:1}{Q1.} \textit{[T/F]} If $A \subset B$ and $B \subset A$, then $A = B$. \sclink{a:ch0:sets:1}{answer}\par\smallskip
  \hypertarget{q:ch0:sets:2}{Q2.} \textit{[T/F]} $\emptyset \subset X$ for every set $X$. \sclink{a:ch0:sets:2}{answer}\par\smallskip
  \hypertarget{q:ch0:sets:3}{Q3.} \textit{[T/F]} ``$A \cap B = \emptyset$'' means $A$ and $B$ share no elements in common. \sclink{a:ch0:sets:3}{answer}\par\smallskip
  \hypertarget{q:ch0:sets:4}{Q4.} \textit{[T/F]} If $A$ has 2 elements and $B$ has 3 elements, then $A \times B$ has 6 elements. \sclink{a:ch0:sets:4}{answer}\par\smallskip
  \hypertarget{q:ch0:sets:5}{Q5.} \textit{Produce.} Given $A = \{1, 2, 3\}$ and $B = \{2, 3, 4\}$, list the elements of $A \cup B$, $A \cap B$, and $A - B$. \sclink{a:ch0:sets:5}{answer}\par\smallskip
  \hypertarget{q:ch0:sets:6}{Q6.} \textit{Produce.} Using set-builder notation, write the Cartesian product $A \times B$ for arbitrary sets $A$ and $B$. \sclink{a:ch0:sets:6}{answer}\par\smallskip
  \hypertarget{q:ch0:sets:7}{Q7.} \textit{[T/F]} The difference set $A - B$ contains exactly those elements that are in $A$ but \emph{not} in $B$. \sclink{a:ch0:sets:7}{answer}
\end{selfcheckbox}
...
\noindent Without going into too much detail we want to briefly introduce the concept of cardinality. Cardinality describes a relationship between sets which compares their relative size.
...
\noindent The last three basic set-theory definitions of this section deal with \textit{Cartesian products}: sets whose elements are ordered pairs (or, more generally, ordered tuples) of elements drawn from one or more underlying sets.
...
\label{sec:ImportantSets}

\begin{selfcheckbox}{0.6}{sec:ManipulatingInequalities}
  \hypertarget{q:ch0:impsets:1}{Q1.} \textit{[T/F]} The interval $[a, b)$ contains $a$ but does not contain $b$. \sclink{a:ch0:impsets:1}{answer}\par\smallskip
  \hypertarget{q:ch0:impsets:2}{Q2.} \textit{[T/F]} $\tfrac{1}{2} \in \Q$ and $\tfrac{1}{2} \in \R$, but $\tfrac{1}{2} \notin \Z$. \sclink{a:ch0:impsets:2}{answer}\par\smallskip
  \hypertarget{q:ch0:impsets:3}{Q3.} \textit{Produce.} Write the set of real numbers $r$ satisfying $-3 \leq r < 5$ in interval notation. \sclink{a:ch0:impsets:3}{answer}\par\smallskip
  \hypertarget{q:ch0:impsets:4}{Q4.} \textit{[T/F]} Every natural number is an integer, every integer is a rational number, and every rational number is a real number. \sclink{a:ch0:impsets:4}{answer}
\end{selfcheckbox}
...
\noindent These families nest as $\N \subset \Z \subset \Q \subset \R$: every natural number is an integer, every integer is rational, and every rational is real, with each containment strict (e.g., $-1 \in \Z \setminus \N$, $\tfrac{1}{2} \in \Q \setminus \Z$, $\sqrt{2} \in \R \setminus \Q$).
...
\begin{selfcheckbox}{0.7}{sec:ContrapositiveExample}
  \hypertarget{q:ch0:ineq:1}{Q1.} \textit{[T/F]} If $a \geq b$ and $c \leq 0$, then $ac \leq bc$. \sclink{a:ch0:ineq:1}{answer}\par\smallskip
  \hypertarget{q:ch0:ineq:2}{Q2.} \textit{[T/F]} If $0 < a \leq b$, then $\tfrac{1}{a} \geq \tfrac{1}{b}$. \sclink{a:ch0:ineq:2}{answer}\par\smallskip
  \hypertarget{q:ch0:ineq:3}{Q3.} \textit{Produce.} Solve $-3x \geq 12$ for $x$, and identify the step where the inequality flips direction. \sclink{a:ch0:ineq:3}{answer}\par\smallskip
  \hypertarget{q:ch0:ineq:4}{Q4.} \textit{Produce.} Given $0 < 2 < 5$, write down the correct chain of inequalities between $0$, $\tfrac{1}{5}$, and $\tfrac{1}{2}$. Cite the rule(s) from this section's table that justify the ordering you chose. \sclink{a:ch0:ineq:4}{answer}
\end{selfcheckbox}
...
\hline
Multiplication &
$ a \geq b \text{ and } c \geq 0 \implies ac \geq bc $ \\
& $ a \geq b \text{ and } c \leq 0 \implies ac \leq bc $ \\
...
\begin{selfcheckbox}{0.8}{sec:RAAExample}
  \hypertarget{q:ch0:contrapos:1}{Q1.} \textit{[T/F]} The contrapositive of ``if $A$ then $B$'' is ``if not $B$ then not $A$,'' and the two are logically equivalent in classical logic. \sclink{a:ch0:contrapos:1}{answer}\par\smallskip
  \hypertarget{q:ch0:contrapos:2}{Q2.} \textit{[T/F]} To prove $A \implies B$ by contrapositive, we assume $\lnot B$ and derive $\lnot A$. \sclink{a:ch0:contrapos:2}{answer}\par\smallskip
  \hypertarget{q:ch0:contrapos:3}{Q3.} \textit{Produce.} State the contrapositive of ``if $n$ is an even integer, then $n + 1$ is odd.'' \sclink{a:ch0:contrapos:3}{answer}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.9}{sec:InductionExample}
  \hypertarget{q:ch0:contra:1}{Q1.} \textit{[T/F]} To prove $A \implies B$ by contradiction, we assume $A$ is true and $B$ is false, then derive a contradiction. \sclink{a:ch0:contra:1}{answer}\par\smallskip
  \hypertarget{q:ch0:contra:2}{Q2.} \textit{Produce.} A friend says ``proof by contradiction starts by assuming the negation of the implication.'' Restate that assumption more carefully: which two things, exactly, do you assume? \sclink{a:ch0:contra:2}{answer}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.10}{sec:InductionPitfall}
  \hypertarget{q:ch0:induction:1}{Q1.} \textit{[T/F]} An induction proof has two parts: a base case and an induction step. \sclink{a:ch0:induction:1}{answer}\par\smallskip
  \hypertarget{q:ch0:induction:2}{Q2.} \textit{[T/F]} The base case of an induction proof must always be $n = 0$. \sclink{a:ch0:induction:2}{answer}\par\smallskip
  \hypertarget{q:ch0:induction:3}{Q3.} \textit{[T/F]} In the induction step, we assume the statement holds for some $n \geq N$ and show it holds for $n + 1$. \sclink{a:ch0:induction:3}{answer}\par\smallskip
  \hypertarget{q:ch0:induction:4}{Q4.} \textit{Produce.} In the induction proof of $\sum_{k=1}^n k = \tfrac{n(n+1)}{2}$ given in this section, what is the base case used, and what does the statement say there? \sclink{a:ch0:induction:4}{answer}
\end{selfcheckbox}
...
\begin{selfcheckbox}{0.12}{sec:ChongInduction}
  \hypertarget{q:ch0:whyinduction:1}{Q1.} \textit{[T/F]} The informal ``$\ldots$'' in the chain $P(N), P(N+1), P(N+2), \ldots$ implicitly appeals to induction itself, so it cannot serve as the justification for induction. \sclink{a:ch0:whyinduction:1}{answer}\par\smallskip
  \hypertarget{q:ch0:whyinduction:2}{Q2.} \textit{[T/F]} The contrapositive-style argument in this section shows that if the base case and induction step both hold, then there cannot exist a smallest $k \geq N$ for which $P(k)$ fails. \sclink{a:ch0:whyinduction:2}{answer}\par\smallskip
  \hypertarget{q:ch0:whyinduction:3}{Q3.} \textit{Produce.} The section's argument denies the existence of a ``first $k$'' for which $P$ fails. Why is denying the existence of a \emph{first} failure enough to conclude $P$ holds for every $n \geq N$? \sclink{a:ch0:whyinduction:3}{answer}
\end{selfcheckbox}
...
We show that repeated application of \textit{modus ponens} is not merely an
...
\newpage
\section{Induction in Fewer Words (Chong/\.Zak An Intro to Optimization 4 ed.)}
\label{sec:ChongInduction}
From Chong \& \.Zak \cite{chong2013introduction}: The principle of mathematical induction may be stated as follows. \\ \\
...
\newpage
\section{Self-Check Answers}
\label{sec:ch0SelfCheckAnswers}

\noindent This section collects answers to the Self-Checks that appear at the start of each section in this chapter. Each answer carries a back-link to its question.

\subsection*{\S\ref{sec:CommonNotation} Common Notation}
\noindent\hypertarget{a:ch0:notation:1}{Q1.} T. ``$A$ only if $B$'' is one of the five standard readings of $A \implies B$ given in this section; the antecedent is $A$, not $B$. \sclink{q:ch0:notation:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:notation:2}{Q2.} T. ``Necessary'' tracks the consequent of an implication; ``sufficient'' tracks the antecedent. \sclink{q:ch0:notation:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:notation:3}{Q3.} T. Both are readings of $A \implies B$: ``sufficient'' tags the antecedent ($A$ on its own is enough to force $B$), and ``necessary'' tags the consequent ($B$ has to hold for $A$ to possibly hold). Reading them paired makes the equivalence obvious. \sclink{q:ch0:notation:3}{back}\par\smallskip
\noindent\hypertarget{a:ch0:notation:4}{Q4.} $\{x \in \Z \mid 22 \leq x < 27\} = \{22, 23, 24, 25, 26\}$. The vertical bar reads as ``for which.'' \sclink{q:ch0:notation:4}{back}\par\smallskip
\noindent\hypertarget{a:ch0:notation:5}{Q5.} (a) ``For every integer $x$, $x^2$ is at least $0$'' (true). Symbols: $\forall$ is ``for every,'' $\in$ is ``is an element of,'' $\Z$ names the integers. (b) ``There exists an integer $x$ for which $x + 5 = 0$'' (true, namely $x = -5$). Symbols: $\exists$ is ``there exists,'' $\ni$ is the element-wise ``for which.'' A reading convention: the comma after the quantification clause (as in $\forall x \in \Z, \, \ldots$) just separates the quantifier-and-domain from the body, so $\forall x \in \Z, P(x)$ reads ``for every $x \in \Z$, $P(x)$ holds.'' \sclink{q:ch0:notation:5}{back}\par\smallskip

\subsection*{\S\ref{sec:LogicalConnectives} Logical Connectives \& Truth Tables}
\noindent\hypertarget{a:ch0:truthtables:1}{Q1.} T. This is \textit{vacuous truth}: an implication with a false antecedent is satisfied with nothing to check. The Purple Motorcycle Club is this section's vehicle for the idea. \sclink{q:ch0:truthtables:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:truthtables:2}{Q2.} T. The whole ``if-then'' expression has its own truth value; it is not meta-machinery outside the logical world. ``Charlie is a human implies Charlie was born on Earth'' is itself either true or false. \sclink{q:ch0:truthtables:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:truthtables:3}{Q3.} T. The two forms agree on every row of the truth table; this section calls the equivalence out explicitly. \sclink{q:ch0:truthtables:3}{back}\par\smallskip
\noindent\hypertarget{a:ch0:truthtables:4}{Q4.} Several examples work, each with an empty domain: (a) ``For every $n \in \N$ with $n < 0$, $n$ is one of my uncles'' (no natural numbers are negative). (b) ``All of my motorcycles are purple,'' when you own zero motorcycles. (c) ``Every element of $\emptyset$ is a number.'' In each case the ``for all'' quantifier has nothing to check, so the statement holds by default. \sclink{q:ch0:truthtables:4}{back}\par\smallskip

\subsection*{\S\ref{sec:NegatingConnectives} Negating Connectives \& Quantifiers}
\noindent\hypertarget{a:ch0:negations:1}{Q1.} T. This is one of DeMorgan's laws (the ``or''-negation form); the negation of an ``or'' is the ``and'' of the negations. \sclink{q:ch0:negations:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:negations:2}{Q2.} $\exists \, x \in \Z \ni x < 0$. (Equivalently in words: ``there exists an integer $x$ for which $x < 0$.'' Negating a universal gives an existential of the negated predicate.) \sclink{q:ch0:negations:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:negations:3}{Q3.} ``It is raining and the sidewalk is not wet.'' The negation of ``if $A$ then $B$'' is ``$A$ and $\lnot B$,'' per the third row of this section's negation table. \sclink{q:ch0:negations:3}{back}\par\smallskip
\noindent\hypertarget{a:ch0:negations:4}{Q4.} $\lnot(A \lor (B \land C)) = \lnot A \land \lnot(B \land C)$ (DeMorgan applied to the outer ``or''), and then $\lnot(B \land C) = \lnot B \lor \lnot C$ (DeMorgan applied to the inner ``and''). So the fully simplified negation is $\lnot A \land (\lnot B \lor \lnot C)$. \sclink{q:ch0:negations:4}{back}\par\smallskip

\subsection*{\S\ref{sec:axioms} A Word About Axioms}
\noindent\hypertarget{a:ch0:axioms:1}{Q1.} T. This is the working definition of ``axiom'' adopted by this book: statements taken as true without justification, serving as the starting point of argument. \sclink{q:ch0:axioms:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:axioms:2}{Q2.} T. Sets have no duplicity: an element is either in a set or it is not, with no notion of multiple copies. So $\{1,2,2,3\} = \{1,2,3\}$. \sclink{q:ch0:axioms:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:axioms:3}{Q3.} F. The repeated $\text{red}$ does not add a new element. The set has 4 elements: $\text{green}, \text{red}, \text{blue}, \text{orange}$. \sclink{q:ch0:axioms:3}{back}\par\smallskip
\noindent\hypertarget{a:ch0:axioms:4}{Q4.} Division is the standard answer: $1 / 2 \notin \Z$. Exponentiation works too: $2^{-1} = 1/2 \notin \Z$. (The four listed facts cover $+$, $-$, additive inverses, and $\cdot$; everything else is fair game for leaving $\Z$ behind.) \sclink{q:ch0:axioms:4}{back}\par\smallskip
\noindent\hypertarget{a:ch0:axioms:5}{Q5.} T. The section explicitly says this book is not a foundations course; sets and the integers $\Z$ (with the listed arithmetic facts) are taken as primitive, with no further justification attempted. \sclink{q:ch0:axioms:5}{back}\par\smallskip

\subsection*{\S\ref{sec:BasicSetTheory} Basic Set Theory Definitions}
\noindent\hypertarget{a:ch0:sets:1}{Q1.} T. This is the definition of set equality used in this book (and the standard one): two sets are equal exactly when each is a subset of the other. \sclink{q:ch0:sets:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:sets:2}{Q2.} T. The statement ``every element of $\emptyset$ is also in $X$'' is vacuously true: the empty set has no elements, so the ``every'' has nothing to check. This is the same vacuous-truth pattern as the Purple Motorcycle Club in \S\ref{sec:LogicalConnectives}. \sclink{q:ch0:sets:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:sets:3}{Q3.} T. By the section's definition: $A$ and $B$ are disjoint exactly when their intersection is the empty set, i.e., they share no elements. \sclink{q:ch0:sets:3}{back}\par\smallskip
\noindent\hypertarget{a:ch0:sets:4}{Q4.} T. The section states $|A \times B| = |A| \cdot |B|$ when both sets are finite. Concretely: $2 \cdot 3 = 6$ ordered pairs. \sclink{q:ch0:sets:4}{back}\par\smallskip
\noindent\hypertarget{a:ch0:sets:5}{Q5.} $A \cup B = \{1, 2, 3, 4\}$ (elements in either); $A \cap B = \{2, 3\}$ (elements in both); $A - B = \{1\}$ (elements in $A$ but not in $B$). \sclink{q:ch0:sets:5}{back}\par\smallskip
\noindent\hypertarget{a:ch0:sets:6}{Q6.} $A \times B = \{(a, b) \mid a \in A \text{ and } b \in B\}$. This is the section's definition of the Cartesian product: the set of all ordered pairs with first coordinate in $A$ and second coordinate in $B$. \sclink{q:ch0:sets:6}{back}\par\smallskip
\noindent\hypertarget{a:ch0:sets:7}{Q7.} T. By the section's definition: $A - B = \{x \in A \mid x \notin B\}$. Equivalent to ``the part of $A$ left over after you remove everything also in $B$.'' \sclink{q:ch0:sets:7}{back}\par\smallskip

\subsection*{\S\ref{sec:ImportantSets} Important Sets \& Interval Notation}
\noindent\hypertarget{a:ch0:impsets:1}{Q1.} T. A square bracket includes its endpoint; a round bracket excludes it. So $a \in [a,b)$ but $b \notin [a,b)$. \sclink{q:ch0:impsets:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:impsets:2}{Q2.} T. $\tfrac{1}{2}$ is rational ($a/b$ with $a=1, b=2$ and $b \neq 0$) and so also real, but it is not an integer. \sclink{q:ch0:impsets:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:impsets:3}{Q3.} $[-3, 5)$. The left endpoint is included ($-3 \leq r$, square bracket); the right endpoint is excluded ($r < 5$, round bracket). \sclink{q:ch0:impsets:3}{back}\par\smallskip
\noindent\hypertarget{a:ch0:impsets:4}{Q4.} T. The familiar nesting chain is $\N \subset \Z \subset \Q \subset \R$. Each containment is strict: $-1 \in \Z$ but $-1 \notin \N$; $\tfrac{1}{2} \in \Q$ but $\tfrac{1}{2} \notin \Z$; $\sqrt{2} \in \R$ but $\sqrt{2} \notin \Q$. \sclink{q:ch0:impsets:4}{back}\par\smallskip

\subsection*{\S\ref{sec:ManipulatingInequalities} Manipulating Inequalities}
\noindent\hypertarget{a:ch0:ineq:1}{Q1.} T. The Multiplication rule in the section's table: multiplying both sides of $a \geq b$ by a non-positive $c$ flips the inequality, giving $ac \leq bc$. \sclink{q:ch0:ineq:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:ineq:2}{Q2.} T. The Reciprocals rule: for $0 < a \leq b$, the reciprocals reverse the inequality, so $\tfrac{1}{a} \geq \tfrac{1}{b}$ (smaller positive number gives larger reciprocal). \sclink{q:ch0:ineq:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:ineq:3}{Q3.} Divide both sides by $-3$ (equivalently, multiply by $-\tfrac{1}{3}$): $x \leq -4$. The flip happens precisely because we multiplied/divided by a negative quantity (the Multiplication or Division rule with $c < 0$). \sclink{q:ch0:ineq:3}{back}\par\smallskip
\noindent\hypertarget{a:ch0:ineq:4}{Q4.} $0 < \tfrac{1}{5} < \tfrac{1}{2}$. Three pieces from the table combine: the third Reciprocals row ($0 < a \leq b \implies \tfrac{1}{a} \geq \tfrac{1}{b}$, applied with $a=2, b=5$) gives $\tfrac{1}{5} < \tfrac{1}{2}$; the first Reciprocals row ($a > 0 \implies \tfrac{1}{a} > 0$) gives $0 < \tfrac{1}{5}$; and Transitivity chains these into the single inequality $0 < \tfrac{1}{5} < \tfrac{1}{2}$. \sclink{q:ch0:ineq:4}{back}\par\smallskip

\subsection*{\S\ref{sec:ContrapositiveExample} Proof by Contrapositive}
\noindent\hypertarget{a:ch0:contrapos:1}{Q1.} T. Both parts are the substance of this section: the contrapositive of $A \implies B$ is $\lnot B \implies \lnot A$, and they share a truth table. \sclink{q:ch0:contrapos:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:contrapos:2}{Q2.} T. That is exactly the structure of a contrapositive proof: assume the negation of the consequent, then derive the negation of the antecedent. \sclink{q:ch0:contrapos:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:contrapos:3}{Q3.} ``If $n+1$ is not odd, then $n$ is not even.'' Equivalently: ``If $n+1$ is even, then $n$ is odd.'' (Negate the consequent, then the antecedent, and swap their order.) \sclink{q:ch0:contrapos:3}{back}\par\smallskip

\subsection*{\S\ref{sec:RAAExample} Proof by Contradiction}
\noindent\hypertarget{a:ch0:contra:1}{Q1.} T. That is the structure of a proof by contradiction in classical logic: assume $A \land \lnot B$, derive $\Rightarrow \Leftarrow$, conclude $A \implies B$. (As this section's footnote notes, the move relies on classical-logic principles like indirect proof; intuitionistic logic treats such proofs differently.) \sclink{q:ch0:contra:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:contra:2}{Q2.} You assume $A$ (the antecedent of the original implication) AND $\lnot B$ (the negation of the consequent). Saying ``assume the negation of $A \implies B$'' is loose shorthand: per the negation table in \S\ref{sec:NegatingConnectives}, the negation of ``if $A$ then $B$'' is ``$A$ and $\lnot B$,'' so the two assumptions are precisely that pair. \sclink{q:ch0:contra:2}{back}\par\smallskip

\subsection*{\S\ref{sec:InductionExample} Proof by Induction Example}
\noindent\hypertarget{a:ch0:induction:1}{Q1.} T. The two parts together establish $P(n)$ for all $n \geq N$. \sclink{q:ch0:induction:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:induction:2}{Q2.} F. The base case can be any natural number $N$; the conclusion is then that $P(n)$ holds for all $n \geq N$. The section's sum-of-integers example uses $n = 1$ as its base. \sclink{q:ch0:induction:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:induction:3}{Q3.} T. That is the structure of the induction step. \sclink{q:ch0:induction:3}{back}\par\smallskip
\noindent\hypertarget{a:ch0:induction:4}{Q4.} The section uses $n = 1$ as the base case. There the statement says $\sum_{k=1}^1 k = 1 = \tfrac{1 \cdot (1+1)}{2} = 1$, which holds by inspection. \sclink{q:ch0:induction:4}{back}\par\smallskip

\subsection*{\S\ref{sec:WhyInductionWorks} Why Induction Works}
\noindent\hypertarget{a:ch0:whyinduction:1}{Q1.} T. The ``$\ldots$'' silently invokes the very induction we are trying to validate; that is the circular reasoning this section warns about. \sclink{q:ch0:whyinduction:1}{back}\par\smallskip
\noindent\hypertarget{a:ch0:whyinduction:2}{Q2.} T. That is the structure of the section's proof: from the base case and induction step, no first failure can exist. \sclink{q:ch0:whyinduction:2}{back}\par\smallskip
\noindent\hypertarget{a:ch0:whyinduction:3}{Q3.} If $P$ failed at any $n \geq N$, the set of failures would be a non-empty subset of the natural numbers, and every non-empty subset of $\N$ has a smallest member (this is a foundational property of $\N$ sometimes called the \textit{well-ordering principle}). So if no \emph{smallest} failure can exist, no failure can exist at all: the failure set is empty, and $P$ holds for every $n \geq N$. \sclink{q:ch0:whyinduction:3}{back}\par\smallskip
