...
\noindent Let's look at some other examples of rings aside from the integers, rationals, reals and complex numbers:
...
\noindent $\Rightarrow$: Assume $R$ is an integral domain; so $R$ has no zero divisors. Now suppose $ab=ac$ with $a\neq 0$.
\begin{align}
...
\begin{enumerate}
    \item Integral domain $R$ has characteristic $0$, denoted $\characteristic(R)=0$, if the only $m\in \Z$ satisfying $1^m=0$ is $m=0$.
...
\newpage
\begin{example}
    $\Z_p$, $p$ prime, has $\characteristic(R)=p$ \ \ \ \ \ ($\underbrace{1+1+\cdots +1}_{p \text{ times}} = 0$)
\end{example}
...
\noindent Take $A,B \in M_2(2\Z)$, then $\exists \ M_1 M_2 \in M_2(\Z) \ni A=2M_1 \ \text{ and } B=2M_2$
...
\begin{enumerate}[label=\roman*)]
    ...
    \noindent Take $r\in \ker\sigma \ $, $\ \ \underbrace{\sigma(r)}_{r+U}\underset{\substack{\uparrow \\ r\in \ker\sigma}}{=} 0+U=U$ \\ Cosets $r+U$ and $0+U$ are identical so $r\in U$, $r$'s coset is $U$.\\
    \textbf{$\supseteq$:} \steezybreak

    \noindent Take $r\in U$. $U$ is the coset $0+U$ so $r\in U \ \implies \ r$'s coset is $0+U$. \steezybreak

    \noindent So $\sigma(r)=r+U=0+U$, now $0+U$ serves as the ``$0$" in $R/U$, $\therefore$ $r\in \ker \sigma$, so $U\subset \ker \sigma$. $\blacksquare$
...
\noindent Furthermore, Lemma 3.7.3 tells us that given $a,b\in R-\{0\}$, $b\not\in R^* \ \implies \ d(a)<d(ab)$
...
\noindent Let $S= \{0\} \cup R^* \cup \{\text{primes}\}\cup \{\text{composites}\}$ (we will show $R-S=\emptyset$)\\ \steezybreak
...
\begin{example}
    ...
    \noindent In $\Z_5$, check for roots: $\cancel{0}, \ \cancel{1}, \underset{\substack{\uparrow \ \  \uparrow \\ root}}{\ 2, \ 3,} \ \cancel{4}$. \\
