...
\paragraph{The empty-domain edge case.}
A subtle question hovers over the universal introduction rule: what if the
domain we are quantifying over has \textit{no} elements? Then there is no
arbitrary $c$ to pick, and the proof move described above does not even
start. In that case the universal $\forall x A$ holds anyway, by the
vacuous-truth reasoning illustrated by
\hyperref[para:purple-motorcycle-club]{the Purple Motorcycle Club back in
Chapter~0}: an ``every"-claim with nothing to check is automatically
satisfied. We mention this for completeness; the domains we quantify over in
this book are never empty in practice, and ordinary $\forall I$ is the move
the reader will actually use.
...
\end{enumerate}
\subsection{Quick Reference for Quantifier and Identity Intro and Elim Rules}
\begin{center}
    \label{ssec:quant_quick}
    \includegraphics[width=\textwidth]{Figures/Quant_rules_no_background.png} \\  \vspace{0.1in} \steezybreak 
    \includegraphics[width=\textwidth]{Figures/Identity_rules_no_background.png} \\ \steezybreak
\end{center}
These seven rules, together with the fourteen propositional rules (\ref{ssec:prop_quick}), complete First-Order logic.
\newpage
...
\begin{definition}[Atomic Formulas (FOL/SOL)]\leavevmode
    \begin{enumerate}
