...
\begin{enumerate}
    \setcounter{enumi}{5} % Continue numbering from previous list
...
\end{enumerate}
\newpage
\subsection{Personal Rings (CHOOSE ONE)}
\begin{enumerate}
    \setlength{\itemsep}{5pt} % Adjusts spacing between items
...
\begin{definition}[Centralizer]
    In a noncommutative ring $R$, the \textit{centralizer} of an element $x$, denoted $\mathcal{C}(x)$, is defined as the set of elements which commute with $x$ under multiplication in $R$:
\begin{align*}
...
\begin{definition}[Annihilator]
    In a ring $R$ with ideal $U$, the \textit{annihilator} of $U$, denoted $\operatorname{Ann}(U)$, is defined as:
\[
...
\begin{definition}[Direct Product of Rings]
    The \textit{direct product of rings} $R$ and $S$ is defined as the Cartesian product:
    \begin{align*}
...
\begin{enumerate}
    ...
    21 + 15i = q(4 - 2i) + r
    \]
    \textul{with $d(r)$ as small as possible}.
