...
\end{definition}
%\textbf{Defn:} A relation on a set $A$ is a statement in two variables which is either true or false for values in $A$\steezybreak\\
\begin{example}
...
\end{example}
%\noindent\textbf{Example 1:} $A=\{\text{people in this room}\}$\\
%$x\sim y \Leftrightarrow x \text{ was born in the same month as } y$\\
%Read the above expression as ``$x$ is related to $y$ if and only if (iff) $x$ was born in the same month as $y$"\steezybreak\\
\begin{example}
$A=\mathbb{R}$\\
...
\end{example}
% \noindent\textbf{Example 2:} $A=\mathbb{R}$\\
% $x\sim y \Leftrightarrow x \geq y$\\
% so if we think of $\sqrt{2}$ and $0$ we have that $\sqrt{2}\geq 0$ so $\sqrt{2}\sim 0$, \\
% However, since $0 \not\geq \sqrt{2} \Rightarrow 0\not\sim \sqrt{2}$ \steezybreak\\
...
\end{enumerate}
\end{definition}
% \noindent \textbf{Defn:} The relation $\sim$ on a set $A$ is said to be:
% \begin{enumerate}[i]
%     \item Reflexive iff $x\sim x \ \forall x \in A$
%     \item Symmetric iff $x\sim y \Rightarrow y\sim x \ \forall \ x,y \in A$
%     \item Transitive iff $x\sim y$ and $y\sim z \Rightarrow x\sim z \ \forall x,y,z \in A$
% \end{enumerate}
% If all three of these properties hold, then $\sim$ is an ``equivalence relation".\\
...
%(if a pair with $x\sim y$ \textit{does} exist we will show that \textit{because} of the way our relation is defined this implies $y\sim x$), 
...
% \noindent\textbf{Example 4:} Prove that $\sim$ is an equivalence relation on $\mathbb{Z}$:\\
% $x\sim y \iff x-y \text{ is even}$\steezybreak\\
% \noindent\textbf{Proof:} To prove we must show $\sim$ is reflexive, symmetric, and transitive on $\mathbb{Z}$.\\
% \textit{Reflexivity:} take $x\in \mathbb{Z}$, then $x-x=0=2*0$, since $0\in \mathbb{Z}$, $x-x$ is even. $\implies x\sim x \ \forall x \in \mathbb{Z}$\\
% \textit{Symmetry:} Assume $x\sim y$, then $x-y$ is even which means $\exists k \in \mathbb{Z}$ such that $x-y=2k$. From here we can see $y-x=-(x-y)=-2k=2(-k)$, since $k\in \mathbb{Z}$, $-k\in \mathbb{Z}$ so $y-x$ is even too so we have $y\sim x$\\
% \textit{Transitivity:} Assume $x\sim y$ and $y \sim z$. These expression imply the existence of two integers. The first expression implies $\exists k\in \mathbb{Z}$ such that $x-y=2k$ and the second expression implies $\exists l \in \mathbb{Z}$ such that $y-z=2l$. Using these we can see that $x-z=x-y+y-z=2k+2l=2*(k+l)$ since $k+l$ is a sum of integers, it is itself an integer so $x\sim y \text{ and } y\sim z \implies x\sim z \ \forall x,y,z \in \mathbb{Z}$. $\ \ \ \blacksquare$
